...
class WarMap4(tk.Tk, object):
    ...
    def _init_map(self):
        ...
        location = []
        for i in range(self.map_h*self.map_w):
            location.append(i)
        random.shuffle(location)
        for i in range(self.red_num):
            location_x_y = location[i]
            x,y = int(location_x_y/self.map_w), int(location_x_y%self.map_w)
            # x,y,id,team,life
            a =  Army(x,y,i,1,'live')
        ...
        for i in range(self.blue_num):
            location_x_y = location[i+self.red_num]
            x, y = int(location_x_y / self.map_w),int(location_x_y % self.map_w)
            # x,y,id,team,life
            a = Army(x, y, i, 2, 'live')
        ...
        for i in range(self.red_num):
            ...
            if self.red_army[i].life!='live':
                ...
                for s in range(len(neighbor)):
                    x,y=neighbor[s][0],neighbor[s][1]
                    if x-1<0 or x+1>self.map_w or y-1<0 or y+1>self.map_h:
                        if x-1<0:
                            qi=qi-1
                        if x+1>self.map_w:
                            qi=qi-1
        ...
        for i in range(self.blue_num):
            ...
            if self.blue_army[i].life!='live':
                ...
                for s in range(len(neighbor2)):
                    x, y = neighbor2[s][0], neighbor2[s][1]
                    if x - 1 < 0 or x + 1 > self.map_w or y - 1 < 0 or y + 1 > self.map_h:
                        if x - 1 < 0:
                            qi = qi - 1
                        if x + 1 > self.map_w:
                        ...
                        if self.env_map[neighbor2[s][1]][neighbor2[s][0]] == 1:
                            qi = qi - 1
                            red=red+1
                        elif self.env_map[neighbor2[s][1]][neighbor2[s][0]] == 2:
                            qi = qi
                            blue=blue+1
    ...
    def fight_result(self):
        ...
        for i in range(self.red_num):
            if self.red_army[i].life != 'live':
                ...
                if chance>self.red_army[i].win_p:
                    self.red_army[i].life = 'dead'
                    self.env_map[y][x] = 0
                    red_killed = red_killed + 1
            # blue fight result
        for i in range(self.blue_num):
            if self.blue_army[i].life != 'live':
                ...
                # army lost fight
                chance = random.uniform(0, 1)
                if chance>self.blue_army[i].win_p:
                    self.blue_army[i].life = 'dead'
                    self.env_map[y][x] = 0
                    blue_killed = blue_killed + 1
        return red_killed,blue_killed
