"""
war_melee come from war_s
different five group,
...
class WarMap4(tk.Tk, object):
    ...
    def _init_map(self):
        ...
        for i in range(self.red_num):
            ...
            # x,y,id,team,life
            if 0<=i<=10:
                a = Army(i*2+1,0,i,1,'live')
            elif 11<i<=20:
                a = Army((i%10)*2, 1, i, 1, 'live')
            elif 21<i<=30:
                a = Army((i%20)*2+1, 2, i, 1, 'live')
            elif 31<i<=40:
                a = Army((i%30)*2, 6, i, 1, 'live')
            elif 41<i<=50:
                a = Army((i%40)*2+1, 5, i, 1, 'live')

            elif 51<=i<=60:
                a = Army((i%50)*2+int(75), 1, i, 1, 'live')
            elif 61<i<=70:
                a = Army((i%60)*2+1+int(75), 2, i, 1, 'live')
            elif 71<i<=80:
                a = Army((i%70)*2+int(75), 3, i, 1, 'live')
            elif 81<i<=90:
                a = Army((i%80)*2+1+int(75), 5, i, 1, 'live')
            elif 91<i<=100:
                a = Army((i%90)*2+1+int(75), 6, i, 1, 'live')
        ...
        for i in range(self.blue_num):
            if 0<=i<=10:
                a = Army(i*2+1,self.map_h-1,i,1,'live')
            elif 11<i<=20:
                a = Army((i%10)*2, self.map_h-2, i, 1, 'live')
            elif 21<i<=30:
                a = Army((i%20)*2+1, self.map_h-3, i, 1, 'live')
            elif 31<i<=40:
                a = Army((i%30)*2, self.map_h-5, i, 1, 'live')
            elif 41<i<=50:
                a = Army((i%40)*2+1, self.map_h-6, i, 1, 'live')

            if 51<=i<=60:
                a = Army((i%50)*2+1+int(75),self.map_h-1,i,1,'live')
            elif 61<i<=70:
                a = Army((i%60)*2+int(75), self.map_h-2, i, 1, 'live')
            elif 71<i<=80:
                a = Army((i%70)*2+1+int(75), self.map_h-3, i, 1, 'live')
            elif 81<i<=90:
                a = Army((i%80)*2+int(75), self.map_h-5, i, 1, 'live')
            elif 91<i<=100:
                a = Army((i%90)*2+1+int(75), self.map_h-6, i, 1, 'live')

            if 101<=i<=110:
                a = Army((i%100)*2+1+int(35),int(self.map_h/2),i,1,'live')
            elif 111<i<=120:
                a = Army((i%110)*2+int(35), int(self.map_h/2)+1, i, 1, 'live')
            elif 121<i<=130:
                a = Army((i%120)*2+1+int(35), int(self.map_h/2)-1, i, 1, 'live')
            elif 131<i<=140:
                a = Army((i%130)*2+int(35), int(self.map_h/2)-3, i, 1, 'live')
            elif 141<i<=150:
                a = Army((i%140)*2+1+int(35), int(self.map_h/2)+3, i, 1, 'live')
    ...
    def _display_window(self):
        ...
        for i in range(self.map_h):
            for j in range(self.map_w):
                x,y=i,j
                if (x+y)<=10 or (x+y)>=188 or (x-y)>=89 or (y-x)>=89:
                    self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                                                                 y * UNIT_PIX, (x + 1) * UNIT_PIX, (y + 1) * UNIT_PIX,
                                                                 fill='Darkred'))
                elif (10>=x>=4 and 10>=y>=4) or (95>=x>=89 and 95>=y>=89) or (95>=x>=89 and 10>=y>=4) or (10>=x>=4 and 95>=y>=89):
                    self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                                                                 y * UNIT_PIX, (x + 1) * UNIT_PIX, (y + 1) * UNIT_PIX,
                                                                 fill='Darkred'))
                elif (95>=x>=5 and 3>=y>=0) or (95>=x>=5 and 99>=y>=96) or (3>=x>=0 and 95>=y>=5)\
                        or (99>=x>=96 and 95>=y>=5):
                    self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                                                                     y * UNIT_PIX, (x + 1) * UNIT_PIX,
                                                                     (y + 1) * UNIT_PIX,
                                                                     fill='red'))
                elif (x+y<=25) or (x+y>=173) or (x-y>=75) or (y-x)>=75:
                    self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                                                                     y * UNIT_PIX, (x + 1) * UNIT_PIX,
                                                                     (y + 1) * UNIT_PIX,
                                                                     fill='IndianRed'))
                elif (95 >= x >= 5 and 8 >= y >= 4) or (95 >= x >= 5 and 96 >= y >= 91) \
                    or (96 >= x >= 91 and 95 >= y >= 5) or (8 >= x >= 4 and 95 >= y >= 5):
                    self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                                                                     y * UNIT_PIX, (x + 1) * UNIT_PIX,
                                                                     (y + 1) * UNIT_PIX,
                                                                     fill='IndianRed'))

                elif (x + y <= 45) or (x + y >= 153) or (x - y >= 55) or (y - x) >= 55:
                    self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                                                                     y * UNIT_PIX, (x + 1) * UNIT_PIX,
                                                                     (y + 1) * UNIT_PIX,
                                                                     fill='OrangeRed'))
                elif (95 >= x >= 5 and 15 >= y >= 9) or (95 >= x >= 5 and 90 >= y >= 84) \
                    or (90 >= x >= 84 and 95 >= y >= 5) or (15 >= x >= 9 and 95 >= y >= 5):
                    self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                                                                     y * UNIT_PIX, (x + 1) * UNIT_PIX,
                                                                     (y + 1) * UNIT_PIX,
                                                                     fill='OrangeRed'))
                ...
                elif (x + y <= 65) or (x + y >= 133) or (x - y >= 35) or (y - x) >= 35:
                    self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                                                                     y * UNIT_PIX, (x + 1) * UNIT_PIX,
                                                                     (y + 1) * UNIT_PIX,
                                                                     fill='Tomato'))
                elif (95 >= x >= 5 and 20 >= y >= 16) or (95 >= x >= 5 and 83 >= y >= 77) \
                    or (83 >= x >= 77 and 95 >= y >= 5) or (20 >= x >= 16 and 95 >= y >= 5):
                    self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                                                                     y * UNIT_PIX, (x + 1) * UNIT_PIX,
                                                                     (y + 1) * UNIT_PIX,
                                                                     fill='Tomato'))
                ...
                # elif (x + y <= 75) or (x + y >= 123) or (x - y >= 75) or (y - x) >= 75:
                #     self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                #                                                      y * UNIT_PIX, (x + 1) * UNIT_PIX,
                #                                                      (y + 1) * UNIT_PIX,
                #                                                      fill='Tomato'))
                ...
                """random color"""
                if (x+y)<=10 or (x+y)>=188 or (x-y)>=89 or (y-x)>=89:
                    x = random.randint(x-25, x+25)
                    y = random.randint(y-25, y+25)
                    self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                                                                 y * UNIT_PIX, (x + 1) * UNIT_PIX, (y + 1) * UNIT_PIX,
                                                                 fill='Darkred'))
                if (95 >= x >= 5 and 8 >= y >= 4) or (95 >= x >= 5 and 96 >= y >= 91) \
                    or (96 >= x >= 91 and 95 >= y >= 5) or (8 >= x >= 4 and 95 >= y >= 5):
                    ...
                    self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                                                                     ...
                                                                     (y + 1) * UNIT_PIX,
                                                                     fill='red'))
                if (95 >= x >= 5 and 15 >= y >= 9) or (95 >= x >= 5 and 90 >= y >= 84) \
                        or (90 >= x >= 84 and 95 >= y >= 5) or (15 >= x >= 9 and 95 >= y >= 5):
                    x = random.randint(x - 5, x + 10)
                    y = random.randint(y - 5, y + 10)
                    self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                                                                     y * UNIT_PIX, (x + 1) * UNIT_PIX,
                                                                     (y + 1) * UNIT_PIX,
                                                                     fill='IndianRed'))
        ...
        for i in range(1400):
            ...
            if 200<=((x-50)*(x-50)+(y-50)*(y-50))<=1600:
                self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                                                             y * UNIT_PIX, (x + 1) * UNIT_PIX,
                                                             (y + 1) * UNIT_PIX,
                                                             fill='Tomato'))
        for i in range(1400):
            ...
            if 300<=((x-50)*(x-50)+(y-50)*(y-50))<=900:
                self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                                                             y * UNIT_PIX, (x + 1) * UNIT_PIX,
                                                             (y + 1) * UNIT_PIX,
                                                             fill='Pink'))
        ...
        for i in range(1400):
            x = random.randint(0, 100)
            y = random.randint(0, 100)
            if 1600<=((x-50)*(x-50)+(y-50)*(y-50))<=3000:
                self.block_draw.append(self.map.create_rectangle(x * UNIT_PIX,
                                                             y * UNIT_PIX, (x + 1) * UNIT_PIX,
                                                             (y + 1) * UNIT_PIX,
                                                             fill='IndianRed'))
